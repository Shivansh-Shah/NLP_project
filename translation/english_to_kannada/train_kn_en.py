...
def extract_pair(example):
    ...
    if isinstance(translation, dict):
        ...
        if kn and en:
            return str(kn).strip(), str(en).strip()
    ...
    # For Samanantar `src`/`tgt`, pick Kannada side as source and English side as target.
...
def evaluate_bleu(model, sp, val_text_ds, device, max_seq_len, max_samples):
    ...
    for idx in indices:
        ...
        pred = greedy_decode(model, sp, row["src"], device=device, max_seq_len=max_seq_len)
...
def main():
    parser = argparse.ArgumentParser(description="Train KN->EN translation model v2 with Samanantar (HF datasets method).")
    parser.add_argument("--dataset-name", default="ai4bharat/samanantar")
    parser.add_argument("--dataset-config", default="kn")
    parser.add_argument("--cache-dir", default="translation/kannada_to_english/kannada_to_english_v2")
    ...
    parser.add_argument("--sp-model-name", default="kannada_to_english_unigram_v2")
